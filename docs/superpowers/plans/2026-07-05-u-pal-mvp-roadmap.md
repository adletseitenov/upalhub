...
- **Визуальный дизайн делает партнёр основателя.** Разработка строит функциональный каркас: нейтральный Tailwind, семантическая разметка, никакой самодеятельной «красоты». Цвета/типографика/отступы — через Tailwind-токены (`tailwind.config` / CSS-переменные), чтобы дизайн партнёра накатывался заменой токенов и стилей, а не переписыванием компонентов.
...
| Дизайн от партнёра | формат передачи (Figma?), какие экраны первыми (лендинг, онбординг, штаб), сроки | макеты желательны к этапу 3, дедлайн — к этапу 5 (запуск) |
...
| 5.6 | Интеграция дизайна партнёра: токены (цвета, типографика, отступы) → Tailwind theme, стилизация экранов по макетам; приоритет — лендинг → онбординг → штаб. Выполняется по мере готовности макетов (можно начинать раньше, параллельно этапам 3–4) | Экраны соответствуют макетам партнёра; функциональность не изменилась (тесты зелёные) |
